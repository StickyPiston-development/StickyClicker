...
# Buttons
def launch():
    subprocess.Popen(["python", "clicker.py", str(int(cps_slider.get())), str(int(ba_slider.get())), str(int(ct_slider.get())), str(mb_box.get())], creationflags=cc_checkbox.get(), stdout=None, shell=False)
def hide():
    app.withdraw()
def show():
    app.deiconify()
